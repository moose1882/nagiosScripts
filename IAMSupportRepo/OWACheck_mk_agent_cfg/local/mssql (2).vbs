...
For Each instId In instIds.Keys
 ...
 ''''''''''''''''''''''''''''''
     RS.Open "SELECT object_name, counter_name, cntr_value FROM sys.dm_os_performance_counters WHERE [object_name] LIKE '%Buffer Manager%' AND [counter_name] = 'Page life expectancy'", CONN
    wscript.echo "<<<mssql_page_life>>>"
    Dim object2Name, counter2Name, instance2Name, value2
    Do While NOT RS.Eof
        object2Name   = Replace(Replace(Trim(RS("object_name")), " ", "_"), "$", "_")
        counter2Name  = LCase(Replace(Trim(RS("counter_name")), " ", "_"))
        instance2Name = Replace(Trim(RS("cntr_value")), " ", "_")
        If instance2Name = "" Then
            instance2Name = "None"
        End If
        value2        = Trim(RS("cntr_value"))
        wscript.echo object2Name & " " & counter2Name & " " & instance2Name & " " & value2
        RS.MoveNext
    Loop
    RS.Close
